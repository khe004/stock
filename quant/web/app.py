...
## 9. cross_asset_mom 跨资产动量（首个跑赢等权基准的动量策略）

**跨资产动量为什么有效**：动量因子的信息量取决于宇宙内资产间的**离散度和低相关性**。
...
**诚实结论（2015-2026 实测）**：
- **策略**（top3 + 绝对动量）：总收益 +237%，夏普 0.80，回撤 -21.5%，Calmar 0.52
- **等权全资产基准**：总收益 +172%，夏普 0.39——策略跑赢基准 = 动量排名真加了信息
- **SPY 长持**：总收益更高——2015-2026 美股独大，任何分散到非美/债/商品的策略都会
  拖累绝对收益。但策略的**风险调整后收益更优**（夏普/Calmar 更高、回撤减半）

这是本平台首个"干净跑赢公平基准"的动量策略：板块动量跑不赢板块等权，个股动量的超额
全靠 NVDA 一只票，而跨资产动量因为宇宙离散度高，排名信号有真信息量。
...
持现金**。旧版硬切 TLT 时，2022 年股债双杀 TLT 也崩，导致 -45% 回撤；改成"TLT 动量也为负
就持现金"后，2022 躲开、回撤降到 -34%。这条"不持下跌的东西（包括避险）"是把回撤压下来的关键。

**诚实结论（2015-2026 实测 + walk-forward）**：
- **进攻档**（top1 + 现金感知避险）：总收益 **+766%**，年化 20.6%，回撤 **-34%**，夏普 0.82，Calmar **0.60**
- **QQQ 长持**（增长标杆）：+635%，年化 18.9%，回撤 -35%，夏普 0.90，Calmar 0.54
- **跑赢 QQQ 的收益，回撤反而略小于 QQQ，Calmar 更高**（夏普略低）——基本达到"进攻但风险可控"
- **walk-forward**：2015-2020 与 2021-2026 两段都追平/跑赢 QQQ，非单段运气
...
**作用范围**：成长 ETF + TLT 避险（`aggressive_growth` 组）。
""")
    st.markdown("""
---
