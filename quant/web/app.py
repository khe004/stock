...
⚠️ **上面的历史演进数字不能直接当预期（2026-09-16 复算）**：月首日仍是四个调仓锚点
（每月第 1/6/11/16 个交易日）里最好的一个（+434%），另外三天约 +333%~+345%，年化跨度 207bp。
去掉这份运气的公平估计 = 4 个错峰 tranche 等权组合 **+362% / 年化 14.0% / 夏普 0.83 / 回撤 -28.7%**。
策略本身站得住（错峰后夏普仍接近最优，回撤明显好于最差单日的 -37.1%），
但绝对收益该按错峰口径而非月首日最佳口径来预期。
...
2. **调仓日 timing luck**（Newfound / Hoffstein-Faber-Braun）。把调仓锚点从每月第 1 个
   交易日错开到第 6/11/16 个交易日，本策略年化收益跨度高达 **562bp**（+5.8%~+11.4%），
   全平台最脆弱；而现用的月首日恰好是四个日期里**最好**的那个（第 16 日只剩 +82%、
   夏普 0.41）。去掉这份运气的公平估计 = 4 个错峰 tranche 等权组合：
   **+179% / 夏普 0.69 / Calmar 0.35**，收益与夏普输给等权基准的 +197% / 0.81 / 0.41。

修正后的错峰 walk-forward 显示明显分裂：2015-2020 超额年化 -2.5%，2020-2026 为 +1.4%；
全历史仍落后公平基准。它不是“两段都输”，也没有稳定跨时段优势。
...
**诚实结论（2026-09-16 复算 + walk-forward）**：
- **进攻档**月首日口径总收益 **+733%**，年化 19.9%，回撤 **-34.3%**，夏普 0.79，Calmar 0.58。
- 4-tranche 错峰口径 **+713% / 年化 19.7% / 回撤 -33.1% / 夏普 0.80 / Calmar 0.59**。
  但单一调仓日年化跨度仍有 441bp，不能再称“timing luck 最稳”；以下比较一律用错峰口径。
...
|---|---|---|---|---|---|
| **策略（错峰口径）** | +713% | +19.7% | **-33.1%** | 0.80 | **0.59** |
| ★ 成长池等权（公平基准） | **+764%** | **+20.3%** | -38.7% | **0.88** | 0.52 |

全历史已经**输池子等权的收益与夏普**，只赢回撤和 Calmar。这是集中押注的标准签名：
回撤形状不同，但没有证据表明 top1 排名增加了风险调整后的信息。

**walk-forward 分裂：前半段略赢、后半段转输**

| 2015-2020 | 总收益 | 夏普 | Calmar | 回撤 |
|---|---|---|---|---|
| 策略（错峰） | **+203%** | 0.91 | 0.63 | -33.1% |
| ★ 成长池等权 | +193% | **0.93** | **0.65** | **-31.0%** |

| 2021-2026 | 总收益 | 夏普 | Calmar | 回撤 |
|---|---|---|---|---|
| 策略（错峰） | +168% | 0.71 | **0.57** | **-32.4%** |
| ★ 成长池等权 | **+195%** | **0.84** | 0.53 | -38.7% |

前半段只略赢收益但输夏普、Calmar 和回撤；后半段输收益与夏普，只赢回撤和 Calmar。
结论不是“某一段证明有效”，而是选择收益高度依赖时段。
...
**实测（2026-09-16 复算，策略数字均为去 timing luck 的错峰口径）**：

| 全历史 | 总收益 | 年化 | 回撤 | 波动 | 夏普 | Calmar |
|---|---|---|---|---|---|---|
| **策略（错峰）** | +212% | 10.2% | **-19.8%** | 10.7% | **0.97** | **0.52** |
| ★ 进攻池等权（公平基准） | +197% | 9.8% | -23.9% | 11.4% | 0.81 | 0.41 |

- **撤回“walk-forward 两段都赢三项”**：2015-2020 超额年化 +0.9%，2020-2026 为 -0.1%
  （近似打平但不是正数）；两段的夏普与回撤仍都优于公平基准。
- 夏普 0.97 也高于 SPY 长持(0.81) 与 QQQ 长持(0.89)。timing luck 跨度 194bp（中等）。

**历史 A/B 曾支持「哨兵能替代自指开关」，但下表除首行外尚未按 2026-09-16 新口径重跑，
只保留为研究记录，不能与当前数字直接比较**：

| 防守方式 | 全历史超额年化 | 夏普差 | 两段是否都为正 |
|---|---|---|---|
| **哨兵单保险（本策略）** | **+0.5%** | **+0.16** | ⚠️ 前正后近似打平 |
| 哨兵 + 自指（HAA 原版双保险） | -0.8% | -0.08 | ❌ |
| 仅自指（现有 cross_asset_mom） | 历史口径 +0.4% | 历史口径 -0.07 | 当前复算一负一正 |
...
- 我们只有 2015 年起的数据，**没有 2008 级别的熊市**；哨兵架构的卖点恰恰是大级别下跌的
  保护，样本内几乎没机会体现。
- 超额主要来自**风险端**（波动、回撤），收益端全历史优势约 +0.5%/年。
- 绝对收益仍**大幅跑输 SPY 长持**——分散到非美/债/商品在这段美股独大的
  历史里必然拖累绝对收益。
