...
def research_status(conn, symbols: list[str], today: date | None = None) -> pd.DataFrame:
    ...
    rows = {s: {"代码": s, "行情日期": None, "基本面日期": None,
                "财报抓取时间": None, "季度三表抓取时间": None} for s in symbols}
    if not symbols:
        return pd.DataFrame()
    ...
    query = f"""SELECT symbol, MAX(captured_at) AS last_date
                FROM financial_statement_snapshots
                WHERE frequency = 'quarterly' AND symbol IN ({placeholders})
                GROUP BY symbol"""
    for row in conn.execute(query, symbols):
        rows[row["symbol"]]["季度三表抓取时间"] = row["last_date"]
    updates = store.load_research_updates(conn, symbols)
    for item in updates.itertuples(index=False):
        label = {"fundamentals": "基本面", "financials": "财报",
                 "quarterly": "季度三表"}.get(item.data_type, item.data_type)
    ...
    quarterly_cutoff = (datetime.combine(today, datetime.min.time(), timezone.utc)
                        - timedelta(days=7)).isoformat()
    for label, cutoff in (("行情日期", price_cutoff), ("基本面日期", fund_cutoff),
                          ("财报抓取时间", fin_cutoff),
                          ("季度三表抓取时间", quarterly_cutoff)):
