...
**排名 vs 缩尾z-score 已做 IC 验证（2026-07-26），维持百分位排名不换**：横截面 Spearman
IC 测试（5/21/60 日前瞻收益，S&P500 全宇宙 124 个月）显示两法排名相关性 0.998+——三个
动量维度本身高度相关，换算法对最终排序影响是噪音级别。且综合分本身在此（幸存者偏差）
个股宇宙上预测力本就很弱（IC 0.004~0.009，ICIR 最高 0.18），与 stock_momentum 的既有
结论一致（个股横截面因子精修在小宇宙上不 transfer）。百分位排名保留其抗极端值优点。
