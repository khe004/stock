...
实测结论（2026-09-16 复算，top1 + 现金感知避险 TLT + BIL 现金等价）：
- 月首日总收益 +733%，年化 19.9%，回撤 -34.3%，夏普 0.79，Calmar 0.58
- （不接 BIL、防御段持 0% 现金时略低；BIL 吃利息略增厚）
- 4-tranche 错峰口径 +713%/年化19.7%/回撤-33.1%/夏普0.80/Calmar0.59；
  单一调仓日年化跨度仍有 441bp，不能再称 timing luck 最稳。

【公平基准对比 2026-09-16】按平台标准（CLAUDE.md 决策 #3），判断"排名有没有加信息"的
可信对比是【与策略共享候选名单的等权持有】，**不是 QQQ**（QQQ 是这十年的事后赢家）。
成长池等权 = 6 只进攻腿 ETF 每日等权。

  全历史        总收益   年化    回撤     夏普   Calmar
  策略（错峰）    +713%  +19.7%  -33.1%  0.80   0.59
  ★成长池等权     +764%  +20.3%  -38.7%  0.88   0.52

- 全历史输池子等权的收益与夏普，只赢回撤与 Calmar；集中排名没有证明增加信息。
- **walk-forward 分裂**：前半段 +203% vs 池等权 +193%，但风险调整指标输；后半段
  +168% vs +195%，只赢回撤与 Calmar。
- 与 backlog 的 4 段拆解（超额集中在 2024-26 AI/半导体行情）互相印证。

口径必须这么说才诚实：
- ✅ 跑赢 QQQ 收益、Calmar 更高 —— 真的，**但 QQQ 不是公平基准**
- ⚠️ 跑赢自己池子的等权 —— **前半段略赢收益，后半段转输，不稳定**
...
class AggressiveMomentum(Strategy):
    ...
    def __init__(self, lookback_days: int = 252, skip_days: int = 21,
                 top_n: int = 1, safe_assets=("TLT",), cash_asset: str = "BIL",
