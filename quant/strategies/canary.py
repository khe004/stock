...
【实测 2026-09-16 复算，一律走 robustness.py 的三根轴】进攻池 = cross_asset_mom 的 9 类资产，
top_n=3，与 cross_asset_mom 同宇宙同参数以便 A/B。策略数字均为**去 timing luck 的错峰口径**。
  ...
  ★进攻池等权     +197%   9.8%  -23.9%  11.4%  0.81  0.41
  → 超额年化 +0.5%，夏普 +0.16，回撤好 4.1pt

  walk-forward： 2015-2020 超额年化 +0.9%；2020-2026 -0.1%（近似打平但不是正数）。
  调仓日 timing luck：年化跨度 246bp，月首日既非最优也非最差。

**撤回“walk-forward 两段都跑赢”的旧说法。** 修正持仓承接与基准成本后，后半段收益略负；
它的主要证据在两段的夏普与回撤，而不是稳定收益超额。
夏普 0.97 也高于 SPY 长持(0.81) 与 QQQ 长持(0.89)。

A/B 支持了「哨兵替代自指开关」这个假设：
  哨兵单保险（本策略）        超额 +0.5%  夏普差 +0.16   ← 前正后近似打平
...
- 超额主要来自**风险端**（波动、回撤），收益端优势约 +0.5%/年。
- 绝对收益仍**大幅跑输 SPY 长持**——和 cross_asset_mom 同理，
