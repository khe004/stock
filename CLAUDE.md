...
- `quant/web/app.py`：十一页面板（市场概览/信号历史/K线/动量排名/市场筛选/AI基建/避险手册/策略评分/策略相关性/回测/策略说明）；
  ...
  内含「⚖️ 杠杆分析」折叠区（把夏普换成收益追 QQQ；融资成本按 BIL+可调价差扣，仅分析用不改实盘信号）
...
2026-09-16 复算：月首日 +434%/夏普0.86，去 timing luck 的错峰口径 +362%/0.83）、
...
跨度 562bp 全平台最脆弱、月首日恰是最优日，错峰口径 +179%/0.69/0.35，收益与夏普输基准）、
aggressive_mom（进攻档：成长 ETF 集中 top1 12-1 动量+现金感知避险+BIL 现金等价；本质集中成长押注。
2026-09-16 复算：错峰口径 +713%/年化19.7%/回撤-33.1%/夏普0.80/Calmar0.59，月首日 +733%。
对★成长池等权（+764%/夏普0.88）已不再赢收益或夏普，只赢回撤与 Calmar；walk-forward 前段略赢收益、
后段明显输收益，仍是集中成长暴露，**"排名加了信息"未被证明**）、
...
进攻宇宙与 cross_asset_mom 完全相同便于 A/B。2026-09-16 复算：错峰口径 +212%/年化10.2%/
回撤-19.8%/夏普0.97/Calmar0.52，vs ★进攻池等权 +197%/9.8%/-23.9%/0.81/0.41。
前半段超额年化 +0.9%，后半段 -0.1%（近似打平），所以撤回“walk-forward 两段都赢”的旧说法；
风险轴仍在两段都占优。诚实边界见 canary.py：21 段风险关闭里 20 段只有 1 个月、
...
9. **稳健性检验回答的是「数字有没有虚高」，不是「有没有 alpha」**（2026-07-27 定调）。
   ...
   - **调仓日 timing luck**（Newfound / Hoffstein-Faber-Braun）：是不是恰好挑中了最好的
     ...
     2026-09-16 复算年化跨度：cross_asset_mom 562bp、aggressive_mom 441bp、
     canary_mom 246bp、dual_momentum 207bp、momentum 133bp、low_vol 130bp。
...
11. **等权是难打败的基线：信号精加工与组合构建优化在我们的小宇宙上都不 work**（2026-07-29 收口）。
    两条 META-LESSON（详见 memory: stock-backlog）：(a) 股票横截面的信号精加工（残差动量/风险调整
    动量/52周高点）需要几百只标的的宽度，在 9-11 资产上丢信息；(b) 组合构建优化（逆波动/最小方差/
    GPM 相关性惩罚）在 9 资产 top-3 上**全部劣于朴素等权**，全历史+两个 walk-forward 段排序一致——
    这是 1/N 结果（DeMiguel-Garlappi-Uppal 2009）：波动/协方差/相关的估计误差压倒优化收益。
    杠杆同理收口（见 memory: stock-leverage）：理论通但需保证金，杠杆 ETF 因加 QQQ-beta+每日重置
    拖累而更差。**edge 只来自能 HOLD 什么（资产类别广度）+ 简单稳健的默认（raw 12-1 + 等权 +
    canary 式防守），不来自把信号/权重磨得更细。** 别再拿"精加工/优化/杠杆"类想法来试，除非
    宇宙规模或数据结构变了（如上真正的 point-in-time 个股宇宙）。
